...
namespace :schedule_task do
  ...
  task schedule_next_task: :environment do
    ...
    todays_tasks.each do |task|
      ...
      new_task = Task.new(name: task.name, time: task.time, repeat: task.repeat, user_id: task.user_id,
                          employee_id: task.employee_id, category: task.category, priority: task.priority)
      case repeat
      when "Daily"
  ...
  task send_reminder_task: :environment do
    ...
    upcoming_tasks.each do |task|
      ...
      user.notifications.create(task_id: task.id, desc: desc, owner_id: task.user_id, seen: false)
    end
  end
end
